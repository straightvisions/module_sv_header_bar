...
for($i = 1; $i < 3; $i++){
	...
	if($module->get_setting('sidebar_' . $i . '_alignment')->get_data()){
		...
		// outer stuff
		$properties['justify-content'] = array(); // row
		...
		foreach($prepared_properties[$i] as $key => $value) {
			...
			$properties['justify-content'][$key]	= 'unset';
			...
			if( isset($module->get_setting('direction')->get_data()[$key]) && $module->get_setting('direction')->get_data()[$key] === 'row' ) {
				...
				$properties['justify-content'][$key] = $value;
			}
	...
	echo $_s->build_css(
		'.sv100_sv_header_bar_wrapper .sv100_sv_header_bar_inner .sv100_sv_sidebar_header_bar_' . $i,
		$properties
	);
}